...
export const demoSpec: StructuredSpec = {
  ...
  outcomeHypotheses: [
    {
      id: 'hyp-1',
      hypothesis: 'Barcode-Scanning reduziert die durchschnittliche Artikelsuchzeit um 80%',
      kpiName: 'Durchschnittliche Suchzeit pro Artikel',
      baselineValue: '15',
      targetValue: '3',
      unit: 'Minuten',
      timeframeWeeks: 12,
      measurementMethod: 'Automatische Zeitmessung: Zeitstempel von Suchanfrage bis Scan-Bestätigung am Zielort',
      dataSource: 'app_builtin',
      confidence: 'high',
      scope: 'full',
    },
    {
      id: 'hyp-2',
      hypothesis: 'Digitale Erfassung steigert die Inventurgenauigkeit auf 99%',
      kpiName: 'Inventurgenauigkeit',
      baselineValue: '85',
      targetValue: '99',
      unit: '%',
      timeframeWeeks: 16,
      measurementMethod: 'Monatlicher Abgleich: System-Bestand vs. physische Stichprobe (100 zufällige Artikel)',
      dataSource: 'app_builtin',
      confidence: 'high',
      scope: 'partial',
      scopeNote: 'Genauigkeit hängt auch von Disziplin der Mitarbeiter ab und davon, ob alle Lagerorte QR-Codes haben',
    },
    {
      id: 'hyp-3',
      hypothesis: 'Eliminierung papierbasierter Prozesse spart 2 FTE an manueller Inventurarbeit',
      kpiName: 'Manuelle Inventurstunden pro Monat',
      baselineValue: '320',
      targetValue: '40',
      unit: 'Stunden/Monat',
      timeframeWeeks: 24,
      measurementMethod: 'Vergleich der geplanten Inventurstunden vor/nach Einführung über HR-Zeiterfassung',
      dataSource: 'external_system',
      confidence: 'medium',
      scope: 'partial',
      scopeNote: 'FTE-Einsparung setzt voraus, dass SAP-Integration korrekt funktioniert und keine Doppelerfassung nötig ist',
    },
  ],
  measurementPlan: {
    ...
    apiEndpoints: [
      {
        path: '/api/v1/outcomes/search-time',
        method: 'GET',
        description: 'Returns average item search time over configurable period',
        responseSchema: {
          kpiName: 'Durchschnittliche Suchzeit pro Artikel',
          currentValue: '',
          baselineValue: '15',
      ...
      {
        path: '/api/v1/outcomes/inventory-accuracy',
        method: 'GET',
        description: 'Returns inventory accuracy rate based on scan coverage vs. physical count',
        responseSchema: {
          kpiName: 'Inventurgenauigkeit',
          currentValue: '',
          baselineValue: '85',
    ...
    builtinTracking: [
      {
        eventName: 'item_search_initiated',
        description: 'Fired when a user starts searching for an item',
        linkedHypothesisId: 'hyp-1',
        aggregation: 'count',
      },
      {
        eventName: 'item_found_scanned',
        description: 'Fired when searched item is scanned at location',
        linkedHypothesisId: 'hyp-1',
        aggregation: 'average',
      },
      {
        eventName: 'scan_completed',
        description: 'Fired on every successful barcode/QR scan',
        linkedHypothesisId: 'hyp-2',
        aggregation: 'count',
      },
      {
        eventName: 'inventory_discrepancy',
        description: 'Fired when physical count differs from system count',
        linkedHypothesisId: 'hyp-2',
        aggregation: 'rate',
      },
    ],
    reviewCadence: 'biweekly',
    firstReviewAfterWeeks: 4,
    escalationThreshold: 'Wenn nach 8 Wochen weniger als 50% der Zielverbesserung erreicht ist, Eskalation an Product Owner und Architect',
  },
};
...
export const interviewQuestions: InterviewQuestion[] = [
  ...
  // Outcome & Messbarkeit - Hypothesen-getrieben
  {
    key: 'success_hypothesis',
    category: 'outcome',
    question: 'What is your main hypothesis? What concrete improvement do you expect? (e.g., "Search time drops from 15 min to 3 min per item")',
    helpText: 'Think in before/after terms: What measurable number changes? Try to include a baseline (today) and target (after go-live)',
    inputType: 'textarea',
    required: true,
    validation: { minLength: 20, maxLength: 2000 },
  },
  {
    ...
    category: 'outcome',
    question: 'How do you measure this today? Do you have baseline numbers?',
    helpText: 'Even rough estimates help. e.g., "We spend about 320 hours/month on manual inventory" or "We currently have ~85% accuracy"',
    inputType: 'textarea',
    required: true,
  },
  {
    key: 'outcome_scope',
    category: 'outcome',
    question: 'Can this solution measure the outcome on its own, or is it part of a bigger picture?',
    helpText: 'Be honest - if the app is only part of the solution, that\'s fine. We\'ll design measurement for what the app CAN track',
    inputType: 'select',
    options: [
      'Fully measurable within the app - the app can track the KPI directly',
      'Partially measurable - the app contributes but other factors play a role too',
      'Hard to measure in-app - success depends mostly on external factors',
    ],
    required: true,
  },
  {
    ...
    category: 'outcome',
    question: 'When should we first check if the hypothesis is confirmed? How often should we review?',
    inputType: 'select',
    options: [
      'After 4 weeks, then biweekly',
      'After 8 weeks, then monthly',
      'After 12 weeks, then quarterly',
    ],
    required: true,
  },
];
